...
int distanceold = 0;
int sumx,sumy,locatex,locatey;
int distance = 0;

  for (int x = 1; x < size_of_p; x++) {
    for (int y = 1; y < size_of_p; y++) {
      if (parking_area[x][y] == 1) {
        for (int i = 1; i < size_of_p; i++) {
          for (int j = 1; j < size_of_p; j++) {
